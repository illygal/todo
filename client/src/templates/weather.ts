...
function shapeBuilder(data: Node, TAPI: typeof TemplateAPI) {
  const color =
    data.payload.weathertype === "sunny"
      ? "#ffa333"
      : data.payload.weathertype === "cloudy"
      ? "#E0E0E0"
      : data.payload.weathertype === "rainy"
      ? "#60B7F0"
      : "#ffffff";
...
<path style="stroke:none;fill:${
    data.payload.ldMode ? "rgb(80, 80, 80)" : "#ffffff"
  ...
  //     <g transform="matrix(0.783807,0,0,0.783807,22.4169,51.6638)">
  ...
  const temp = TAPI.TextCollection(
    ...
    [
      ...
      TAPI.ShapeStyle("stroke-width", "0"),
      TAPI.ShapeStyle("class", "gly_text"),
      TAPI.ShapeStyle("fill", "rgb(80, 80, 80)", data.payload.ldMode),
      TAPI.ShapeStyle("fill", "#ffffff", !data.payload.ldMode),
